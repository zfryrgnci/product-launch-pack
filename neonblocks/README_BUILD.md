# Neon Blocks — Production Build & Publish Guide
...
│    └─ index.html
├─ app/src/main/java/com/refaz/neonblocks/
...
https://admob.google.com → add app "Neon Blocks" → create a Banner, an
Interstitial, and a Rewarded unit. Copy the 4 IDs (App ID + 3 unit IDs).

**(c) Paste your IDs in two spots, flip one switch:**
- `app/src/main/java/com/refaz/neonblocks/AdConfig.kt` → paste the 3 unit IDs into
...
- **Revenue is never guaranteed.** The code and monetization are done correctly;
  ...
  money button.
- **Package name `com.refaz.neonblocks` is permanent** once published. Change it in
