...
class MainActivity : Activity() {
    ...
    private fun startApp() {
        ...
        billing = BillingManager(applicationContext) { removed ->
            runOnUiThread {
                ...
                webView.evaluateJavascript("window.NeonUnravel && window.NeonUnravel.onAdsRemovedChanged($removed);", null)
            }
        }
    ...
    inner class GameBridge {
        ...
        @JavascriptInterface fun showRewarded() {
            ads.showRewarded { granted ->
                runOnUiThread {
                    webView.evaluateJavascript("window.NeonUnravel && window.NeonUnravel.onReward($granted);", null)
                }
            }
        }
    ...
    override fun onPause() {
        super.onPause()
        webView.evaluateJavascript("window.NeonUnravel && window.NeonUnravel.onPause();", null)
        webView.onPause()
    }
    ...
    override fun onResume() {
        super.onResume()
        webView.onResume()
        webView.evaluateJavascript("window.NeonUnravel && window.NeonUnravel.onResume();", null)
    }
